...
You have a dedicated workspace with this structure:
  ...
  - enterprise_info/ → Shared company information (read-only, synced by admin)
  - shared/          → Public read-write area for ALL agents (cross-agent collaboration)
  - peers/<name>/    → Read-only access to other agents' workspaces (browse their files)
...
11. **Multimodal in code (video/image/audio):**
   - Sandbox has `OPENAI_API_KEY` and `OPENAI_BASE_URL` pre-set to Wanqing Gemini proxy.
   - Use model `ep-3vrrg5-1772715777505520100` for video/image/audio analysis.
   - Call via openai SDK: `openai.OpenAI(api_key=os.environ["OPENAI_API_KEY"], base_url=os.environ["OPENAI_BASE_URL"])`
   - Pass video as: `{"type":"video_url","video_url":{"url":"..."}}`
   - **NEVER use GEMINI_API_KEY or call Google API directly.**""")

...
# Agent with full exec_shell access (龙虾大脑)
_SUPER_AGENT_ID = "f456a3cf-4240-4050-9fb6-82df763c4c75"
...
AGENT_TOOLS = [
    {
        ...
        "function": {
            "name": "list_files",
            "description": "List files and folders. Paths: my workspace (default), enterprise_info/ (company info), shared/ (public read-write for all agents), peers/<agent_name>/ (read other agents' workspaces).",
            "parameters": {
                "type": "object",
                "properties": {
                    "path": {
    ...
    },
    {
        ...
        "function": {
            "name": "read_file",
            "description": "Read file contents. Supports: my workspace files, enterprise_info/ (company info), shared/ (public area), peers/<agent_name>/ (read other agents' files read-only).",
            "parameters": {
                "type": "object",
                "properties": {
                    "path": {
    ...
    },
    {
        ...
        "function": {
            "name": "write_file",
            "description": "Write or update a file. Can write to: my workspace, shared/ (public area for all agents). Cannot write to peers/ or enterprise_info/ (read-only).",
            "parameters": {
                "type": "object",
                "properties": {
                    "path": {
    ...
    },
    {
        ...
        "function": {
            "name": "reddit_fetch",
            "description": "直接抓取 Reddit 子版块帖子（hot/new/top），无需 OAuth，服务端执行不受沙箱限制。适合 r/MachineLearning、r/LocalLLaMA、r/artificial 等 AI 相关内容。",
            "parameters": {
                "type": "object",
                "properties": {
                    "subreddit": {"type": "string", "description": "子版块名称如 LocalLLaMA（不含 r/）"},
                    "sort": {"type": "string", "enum": ["hot","new","top","rising"], "description": "排序方式默认hot"},
                    "limit": {"type": "integer", "description": "条数上限25，默认15"}
                },
                "required": ["subreddit"]
            }
        }
    },
    {
        ...
        "function": {
            "name": "rss_fetch",
            "description": "抓取任意 RSS/Atom 订阅源。适合量子位、36氪、技术博客等。比 jina_read 更适合批量抓标题列表。",
            "parameters": {
                "type": "object",
                "properties": {
                    "url": {"type": "string", "description": "RSS/Atom 订阅源完整 URL"},
                    "limit": {"type": "integer", "description": "条数上限30，默认10"}
                },
                "required": ["url"]
            }
        }
    ...
    },
    {
        ...
        "function": {
            "name": "exec_shell",
            "description": "[SUPER AGENT ONLY] Execute any shell command on the host machine. Full access: install packages, run scripts, manage files, call APIs, restart services. Use with care.",
            "parameters": {
                "type": "object",
                "properties": {
                    "command": {
                        "type": "string",
                        "description": "Shell command to execute, e.g.: 'pip install moviepy', 'ls -la', 'python3 script.py'",
                    },
                    "timeout": {
                        "type": "integer",
...
async def get_agent_tools_for_llm(agent_id: uuid.UUID) -> list[dict]:
            ...
            if result:
                ...
                for t in _always_tools:
                    if t["function"]["name"] not in db_tool_names:
                        result.append(t)
                # Inject exec_shell for super agent only
                if str(agent_id) == _SUPER_AGENT_ID:
                    exec_tool = next((t for t in AGENT_TOOLS if t["function"]["name"] == "exec_shell"), None)
                    if exec_tool and "exec_shell" not in db_tool_names:
                        result.append(exec_tool)
                return result
    ...
    # Remove exec_shell for non-super agents
    if str(agent_id) != _SUPER_AGENT_ID:
        tools = [t for t in tools if t["function"]["name"] != "exec_shell"]
    return tools
...
async def execute_tool(
    ...
    try:
        if tool_name == "list_files":
        ...
        elif tool_name == "reddit_fetch":
            result = await _reddit_fetch(arguments)
        elif tool_name == "rss_fetch":
            result = await _rss_fetch(arguments)
            ...
            # Only available to super agent
            if str(agent_id) != _SUPER_AGENT_ID:
                result = "❌ exec_shell is restricted to authorized agents only."
            else:
                result = await _exec_shell(arguments)
...
async def _execute_via_smithery_connect(mcp_url: str, tool_name: str, arguments: dict, config: dict, agent_id=None) -> str:
    ...
    headers = {
        ...
        "Accept": "application/json, text/event-stream",  # MCP Streamable HTTP 协议要求
    }
...
def _resolve_peer_path(rel_path: str):
    """Resolve peers/<agent_name>/... to actual agent workspace path (read-only)."""
    parts = rel_path.split("/", 2)  # ['peers', '<name>', ...rest]
    if len(parts) < 2:
        return None
    peer_name = parts[1]
    rest = parts[2] if len(parts) > 2 else ""
    # Find agent directory by name
    import asyncio
    try:
        loop = asyncio.get_event_loop()
    except RuntimeError:
        return None
    # Scan agent_data for matching agent name via soul.md or directory listing
    for entry in WORKSPACE_ROOT.iterdir():
        if not entry.is_dir() or entry.name in ("enterprise_info", "shared", "_archived"):
            continue
        soul_file = entry / "soul.md"
        if soul_file.exists():
            content = soul_file.read_text(encoding="utf-8", errors="replace")[:500]
            if peer_name.lower() in content.lower():
                if rest:
                    return str(entry.name) + "/" + rest
...
def _list_files(ws: Path, rel_path: str) -> str:
    ...
    # Handle shared/ as public read-write directory
    elif rel_path and rel_path.startswith("shared"):
        target = (WORKSPACE_ROOT / rel_path).resolve()
        shared_root = (WORKSPACE_ROOT / "shared").resolve()
        if not str(target).startswith(str(shared_root)):
            return "Access denied for this path"
    # Handle peers/<name>/ for read-only access to other agents' workspace
    elif rel_path and rel_path.startswith("peers/"):
        target = (WORKSPACE_ROOT / _resolve_peer_path(rel_path)).resolve()
        if target is None:
    ...
    items = []
    # If listing root, also show enterprise_info, shared, and peers entries
    if not rel_path:
        enterprise_dir = WORKSPACE_ROOT / "enterprise_info"
        if enterprise_dir.exists():
            items.append("  📁 enterprise_info/ (shared company info, read-only)")
        shared_dir = WORKSPACE_ROOT / "shared"
        if shared_dir.exists():
            items.append("  📁 shared/ (public read-write area for all agents)")
        items.append("  📁 peers/ (read-only access to other agents' workspaces)")
...
def _read_file(ws: Path, rel_path: str) -> str:
    ...
    # Handle shared/ as public read-write directory
        ...
        file_path = (WORKSPACE_ROOT / rel_path).resolve()
        shared_root = (WORKSPACE_ROOT / "shared").resolve()
        if not str(file_path).startswith(str(shared_root)):
            return "Access denied for this path"
    # Handle peers/<name>/ for read-only access to other agents
    elif rel_path and rel_path.startswith("peers/"):
        resolved = _resolve_peer_path(rel_path)
        if resolved is None:
            return "Peer agent not found"
        file_path = (WORKSPACE_ROOT / resolved).resolve()
    else:
        file_path = (ws / rel_path).resolve()
...
def _write_file(ws: Path, rel_path: str, content: str) -> str:
    ...
    # Block writes to peers/ (read-only)
    if rel_path and rel_path.startswith("peers/"):
        return "Access denied: peers/ is read-only. Use shared/ for cross-agent collaboration."
    # Allow writes to shared/ directory
    if rel_path and rel_path.startswith("shared"):
        file_path = (WORKSPACE_ROOT / rel_path).resolve()
        shared_root = (WORKSPACE_ROOT / "shared").resolve()
        if not str(file_path).startswith(str(shared_root)):
            return "Access denied for this path"
...
_DANGEROUS_BASH = [
    ...
    "nc ", "ncat ", "ssh ", "scp ",
    # curl/wget allowed for API calls
    "python3 -c", "python -c",
]
...
_DANGEROUS_PYTHON_IMPORTS = [
    ...
    # Network libs allowed for API calls: socket, http.client, urllib, requests, openai, dashscope
]
...
async def _exec_shell(arguments: dict) -> str:
    """Execute arbitrary shell command on host machine (super agent only)."""
    import asyncio
...
async def _execute_code(ws: Path, arguments: dict) -> str:
    ...
    if language == "python":
        ext = ".py"
        cmd_prefix = ["/Users/zhuzhiheng/Desktop/Clawith-main/backend/.venv/bin/python3"]
    elif language == "bash":
        ext = ".sh"
        cmd_prefix = ["bash"]
    elif language == "node":
    ...
    try:
        ...
        # Inject Wanqing API credentials so agents can call LLM APIs in code
        from app.config import get_settings as _gs
        _s = _gs()
        safe_env["WQ_API_KEY"] = getattr(_s, 'WQ_API_KEY', 'iax36glyeuxl5yopgha1dn1v256q74d61qdh')
        safe_env["WQ_API_BASE"] = "https://wanqing-api.corp.kuaishou.com/api/gateway/v1/endpoints/"
        safe_env["OPENAI_API_KEY"] = safe_env["WQ_API_KEY"]
        safe_env["OPENAI_BASE_URL"] = safe_env["WQ_API_BASE"]
...
# ─── Reddit & RSS 工具（服务端直接请求，绕过沙箱限制）───────────────────────

async def _reddit_fetch(arguments: dict) -> str:
    """直接从 Reddit RSS 抓取帖子，无需 OAuth。服务端执行，不受 agent 沙箱限制。"""
    import httpx, re
    from xml.etree import ElementTree as ET

    subreddit = arguments.get("subreddit", "MachineLearning").strip().lstrip("r/")
    sort = arguments.get("sort", "hot")  # hot, new, top, rising
    limit = min(int(arguments.get("limit", 15)), 25)

    url = f"https://www.reddit.com/r/{subreddit}/{sort}/.rss?limit={limit}"
    headers = {"User-Agent": "Mozilla/5.0 LobsterNewsBot/2.0 (content aggregator)"}
    ...
    try:
        ...
        if resp.status_code == 404:
            return f"❌ 子版块 r/{subreddit} 不存在"
        if resp.status_code != 200:
            return f"❌ Reddit 请求失败 HTTP {resp.status_code}"
        ...
        for entry in entries[:limit]:
            ...
            content_text = re.sub(r"\s+", " ", content_text)[:300]

            # 提取评论数和分数（在 content HTML 里）
            score_match = re.search(r"(\d+) points?", content_html)
            comments_match = re.search(r"(\d+) comments?", content_html)
            score = score_match.group(1) if score_match else "?"
            comments = comments_match.group(1) if comments_match else "?"

            posts.append(
                f"**{title}**\n"
                f"  作者: u/{author} | 👍 {score} | 💬 {comments} 评论\n"
                f"  链接: {link}\n"
                f"  摘要: {content_text[:200]}"
            )

        if not posts:
            return f"r/{subreddit} 暂无帖子"

        return (
            f"📋 r/{subreddit} {sort} 帖子（{len(posts)}条）\n\n"
            + "\n\n".join(posts)
        )

    except ET.ParseError as e:
        return f"❌ RSS 解析失败: {str(e)[:100]}"
    except Exception as e:
        return f"❌ reddit_fetch 错误: {str(e)[:200]}"


async def _rss_fetch(arguments: dict) -> str:
    """通用 RSS/Atom 订阅源抓取。适用于技术博客、新闻网站等。"""
    import httpx, re
    from xml.etree import ElementTree as ET

    feed_url = arguments.get("url", "").strip()
    limit = min(int(arguments.get("limit", 10)), 30)

    if not feed_url:
        return "❌ 请提供 RSS 订阅源 URL"

    headers = {"User-Agent": "Mozilla/5.0 LobsterNewsBot/2.0"}
    ...
    try:
        ...
        if resp.status_code != 200:
            return f"❌ RSS 请求失败 HTTP {resp.status_code}: {feed_url}"
        ...
        if root.tag == "rss" or "rss" in root.tag:
            ...
            if channel:
                for item in channel.findall("item")[:limit]:
                    title = getattr(item.find("title"), "text", "无标题") or "无标题"
        ...
        if not items:
            return f"❌ 未找到条目，请检查 URL 是否为有效的 RSS/Atom 订阅源"

        return f"📰 RSS 订阅（{len(items)}条）: {feed_url}\n\n" + "\n\n".join(items)

    except ET.ParseError:
        return f"❌ 不是有效的 RSS/Atom 格式: {feed_url}"
    except Exception as e:
        return f"❌ rss_fetch 错误: {str(e)[:200]}"
